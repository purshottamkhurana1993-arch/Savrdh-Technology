...
function buildPosterPdf(theme: 'dark' | 'emerald' = 'dark') {
  const doc = new jsPDF({
  ...
  const specBadges = [
    ...
    { title: 'Multi-Tenant Isolated', sub: 'Savrdh Cloud Suite' }
  ];
...
const pdfBuffer = buildPosterPdf('dark');
...
const fileName = 'FieldSure_Coming_Soon_Poster_Savrdh.pdf';
fs.writeFileSync(path.join(publicDir, fileName), Buffer.from(pdfBuffer));
console.log(`Saved poster PDF to public/${fileName}`);

fs.writeFileSync(path.join(distDir, fileName), Buffer.from(pdfBuffer));
console.log(`Saved poster PDF to dist/${fileName}`);

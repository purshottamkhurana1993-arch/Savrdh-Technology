        ...
        <div className="space-y-4">
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-xs text-emerald-900 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span>
                <strong>Privacy Policy Compliance:</strong> Only displaying employees who have an active duty session and verified consent. Tracking ceases automatically after punch-out.
              </span>
            </div>
            <span className="font-mono text-[11px] font-bold text-emerald-800 shrink-0">4 Active Officers Tracked</span>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Left Col: Interactive SVG Simulation Map */}
            <div className="lg:col-span-2 bg-slate-900 rounded-2xl border border-slate-800 p-4 shadow-xl flex flex-col justify-between relative overflow-hidden min-h-[480px]">
              
              {/* Map Title & Filter */}
              <div className="flex items-center justify-between z-10 bg-slate-900/90 backdrop-blur-xs p-3 rounded-xl border border-slate-800">
                <div className="flex items-center gap-2">
                  <Navigation className="w-4 h-4 text-emerald-400 animate-pulse" />
                  <span className="text-xs font-bold text-white">Live Field Grid (Delhi NCR Telemetry)</span>
                </div>
                <div className="flex items-center gap-2 text-[11px] text-slate-400">
                  <span className="inline-block w-2 h-2 rounded-full bg-emerald-500"></span> Live Duty
                  <span className="inline-block w-2 h-2 rounded-full bg-amber-500 ml-2"></span> On Break
                </div>
              </div>

              {/* Mock Map Canvas (SVG Vector Roads & Pings) */}
              <div className="relative w-full h-[360px] my-2 bg-slate-950 rounded-xl overflow-hidden border border-slate-800/80 flex items-center justify-center">
                {/* SVG Map Grid Lines */}
                <svg className="w-full h-full absolute inset-0 opacity-40">
                  ...
                  {/* Route Trajectory Breadcrumb for selected employee */}
                  <path d="M 120 260 L 240 210 L 380 170 L 480 140" fill="none" stroke="#10b981" strokeWidth="2.5" strokeDasharray="3 3" />
                </svg>

                {/* Animated Map Pins */}
                {/* Pin 1: Rahul Sharma (Connaught Place) */}
                <div 
                  onClick={() => setSelectedEmployeeForMap('emp-rahul-sharma')}
                ...
                >
                  ...
                  <div className="absolute left-1/2 -translate-x-1/2 mt-1.5 bg-slate-900 border border-slate-700 text-white text-[10px] font-semibold px-2 py-0.5 rounded shadow-md whitespace-nowrap">
                    Rahul Sharma (CP Outlet)
                  </div>
                </div>

                {/* Pin 2: Priya Verma (Gurugram Cyber Hub) */}
                <div 
                  onClick={() => setSelectedEmployeeForMap('emp-priya-verma')}
                ...
                {/* Pin 3: Amit Kumar (Faridabad - On Break) */}
                <div 
                  onClick={() => setSelectedEmployeeForMap('emp-amit-kumar')}
                ...
                {/* Pin 4: Neha Singh (Sohna Road) */}
                <div 
                  onClick={() => setSelectedEmployeeForMap('emp-neha-singh')}
              ...
              <div className="z-10 bg-slate-900/90 p-2.5 rounded-xl border border-slate-800 flex items-center justify-between text-xs text-slate-400">
                <span>Map Provider: Google Maps Platform SDK (Mocked Vector Layer)</span>
                <span className="text-emerald-400 font-medium">GPS Accuracy: ±3.8m • Refresh: 5s</span>
              </div>
            </div>

            {/* Right Col: Selected Employee Live Route Trace & Details */}
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between space-y-4">
              <div>
                <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                  <h3 className="text-sm font-bold text-slate-900">Active Officer Details</h3>
                ...
                <div className="mt-4 p-3 bg-slate-50 rounded-xl border border-slate-200/80 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[11px] font-mono text-slate-500">AKBS-FLD-102</span>
                  </div>
                  <p className="text-xs text-slate-600">Senior Farm Area Officer • North Region</p>
                  
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200/60 text-xs">
                    <div>
                      <span className="text-slate-400 text-[10px] block">Punch-In Time</span>
                      <span className="font-semibold text-slate-800">08:54 AM (Noida)</span>
                    </div>
                    <div>
                      <span className="text-slate-400 text-[10px] block">Battery Status</span>
                      <span className="font-semibold text-emerald-700 flex items-center gap-1">
                        <Battery className="w-3.5 h-3.5" /> 74% Good
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 space-y-2.5">
                  ...
                  <div className="space-y-2 text-xs">
                    {routePoints.map((pt, idx) => (
                      <div key={pt.id} className="p-2.5 rounded-lg border border-slate-100 bg-slate-50/70 space-y-1">
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-900 flex items-center justify-between">
                <span>View Raw Audit GeoJSON</span>
                <button 
                  onClick={() => showToast('GeoJSON route log exported to console.')}
                  className="font-bold text-blue-700 hover:underline text-[11px]"
                >
                  Export Log
                </button>
              </div>

...
import { DutySession, FieldVisit, RoutePoint, FieldTask } from '../../types';
...
export const LiveDutyGoogleMap: React.FC<LiveDutyGoogleMapProps> = ({
  ...
  // Selected task for opening Full Live En-Route Radar Modal
  ...
  useEffect(() => {
    ...
    // 5. Plot En-Route Task Destination & Navigation Corridor for Selected Employee
    const activeTask = tasks.find(
      t => t.assignedToUserId === selectedEmployeeId && (t.status === 'in_progress' || t.status === 'pending') && t.targetLat && t.targetLng
    );
    const selectedSession = liveSessions.find(s => s.userId === selectedEmployeeId);

    if (activeTask && selectedSession && activeTask.targetLat && activeTask.targetLng) {
      // 100m Target Geofence Circle
      const targetCircle = L.circle([activeTask.targetLat, activeTask.targetLng], {
        radius: activeTask.targetGeofenceRadiusMeters || 100,
        color: '#2563eb',
        fillColor: '#3b82f6',
        fillOpacity: 0.2,
        weight: 2,
      ...
      const targetPinIcon = L.divIcon({
        className: 'custom-task-target-pin',
        html: `
          <div style="cursor: pointer; display: flex; flex-direction: column; align-items: center;">
            <div style="background: #2563eb; color: white; padding: 4px 10px; border-radius: 20px; font-size: 11px; font-weight: 800; border: 2px solid white; box-shadow: 0 4px 14px rgba(37,99,235,0.45); display: flex; align-items: center; gap: 4px; white-space: nowrap;">
              <span>🎯</span>
              <span>${activeTask.clientName}</span>
            </div>
            <div style="width: 2px; height: 12px; background: #2563eb;"></div>
            <div style="width: 8px; height: 8px; border-radius: 50%; background: #2563eb; border: 2px solid white;"></div>
          </div>
        `,
        iconSize: [160, 48],
        iconAnchor: [80, 48]
      ...
      const targetPinMarker = L.marker([activeTask.targetLat, activeTask.targetLng], { icon: targetPinIcon })
        .bindPopup(`
          <div style="padding: 6px; font-family: system-ui, sans-serif; min-width: 220px;">
            <div style="font-size: 10px; font-weight: 800; color: #2563eb; text-transform: uppercase;">🎯 EN-ROUTE TASK DESTINATION</div>
            <div style="font-size: 13px; font-weight: 800; color: #0f172a; margin-top: 2px;">${activeTask.clientName}</div>
            <div style="font-size: 11px; color: #64748b; margin-top: 2px;">${activeTask.clientAddress}</div>
            <div style="margin-top: 6px; font-size: 11px; color: #334155;"><strong>Deliverable:</strong> ${activeTask.title}</div>
            <div style="margin-top: 4px; font-size: 11px; color: #2563eb; font-weight: bold;">Geofence: ${activeTask.targetGeofenceRadiusMeters || 100}m Radius</div>
          </div>
        `);

      // En-Route Dotted Navigation Corridor
      const enRouteLine = L.polyline([
        [selectedSession.lat, selectedSession.lng],
        [activeTask.targetLat, activeTask.targetLng]
      ], {
        color: '#3b82f6',
        weight: 4,
        opacity: 0.9,
      ...
      group.addLayer(targetPinMarker);
      group.addLayer(enRouteLine);
    }

  }, [liveSessions, fieldVisits, tasks, selectedEmployeeId, mapEngine, myRealLocation, routePoints, activeFilter]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 flex flex-col space-y-3">
          ...
          <div className="relative w-full h-[540px] rounded-2xl overflow-hidden border border-slate-300 shadow-md bg-slate-100">
            ...
            {/* Floating Top En-Route Navigation HUD Pill */}
            {(() => {
              const activeTask = tasks.find(
                t => t.assignedToUserId === selectedEmployeeId && (t.status === 'in_progress' || t.status === 'pending') && t.targetLat && t.targetLng
              );
              ...
              if (activeTask && selectedSession && activeTask.targetLat && activeTask.targetLng) {
                const telemetry = evaluateEnRouteTelemetry(
                  selectedSession.lat,
                  selectedSession.lng,
                  selectedSession.speedKmH,
                  activeTask.targetLat,
                  activeTask.targetLng,
                  activeTask.initialTripDistanceMeters || 2800,
                  activeTask.targetGeofenceRadiusMeters || 100
                );
                return (
                  <div className="absolute top-3 left-3 right-3 z-30 flex items-center justify-between gap-2 p-2.5 rounded-xl bg-slate-900/90 backdrop-blur-md border border-slate-700/80 text-white shadow-xl">
                    <div className="flex items-center gap-2.5 overflow-hidden">
                      ...
                      <div className="truncate">
                        <div className="flex items-center gap-1.5 text-xs font-bold">
                          <span className="text-emerald-400">En-Route Corridor:</span>
                          <span className="truncate">{activeTask.clientName}</span>
                        </div>
                        <p className="text-[11px] text-slate-300">
                          {telemetry.statusText} • <strong className="text-emerald-400">{telemetry.formattedDistance}</strong> away ({telemetry.formattedEta})
                        </p>
                      </div>
                    </div>

                    <button
                      onClick={() => setSelectedTaskForRadar(activeTask)}
                      className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold shrink-0 flex items-center gap-1 shadow-xs transition-colors"
                    >
                      <Radio className="w-3.5 h-3.5" /> Open Radar HUD
                    </button>
                  </div>
                );
              }
            ...
            {mapEngine === 'google_maps' && activeApiKey && (
              <APIProvider apiKey={activeApiKey}>
                ...
                >
                  ...
                  {/* En-Route Task Destination & Corridor Polyline in Google Maps */}
                  {(() => {
                    const activeTask = tasks.find(
                      t => t.assignedToUserId === selectedEmployeeId && (t.status === 'in_progress' || t.status === 'pending') && t.targetLat && t.targetLng
                    );
                    ...
                    if (activeTask && selectedSession && activeTask.targetLat && activeTask.targetLng) {
                      return (
                        <>
                          <GoogleMapPolyline 
                            path={[
                              { lat: selectedSession.lat, lng: selectedSession.lng },
                              { lat: activeTask.targetLat, lng: activeTask.targetLng }
                            ]} 
                            color="#2563eb" 
                          />
                          <AdvancedMarker
                            position={{ lat: activeTask.targetLat, lng: activeTask.targetLng }}
                            title={`Destination: ${activeTask.clientName}`}
                          >
                            <div className="cursor-pointer px-2.5 py-1 rounded-full bg-blue-600 text-white font-bold text-xs border-2 border-white shadow-xl flex items-center gap-1.5 whitespace-nowrap">
                              <span>🎯</span>
                              <span>{activeTask.clientName}</span>
                            </div>
                          </AdvancedMarker>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-xs flex flex-col justify-between space-y-4">
          <div>
            ...
            {/* Live Destination Proximity & En-Route Heading Tracker Card */}
            {(() => {
              const activeTask = tasks.find(
                t => t.assignedToUserId === selectedEmployeeId && (t.status === 'in_progress' || t.status === 'pending') && t.targetLat && t.targetLng
              );
              if (activeTask && activeTask.targetLat && activeTask.targetLng) {
                const telemetry = evaluateEnRouteTelemetry(
                  selectedEmployeeSession.lat,
                  selectedEmployeeSession.lng,
                  selectedEmployeeSession.speedKmH,
                  activeTask.targetLat,
                  activeTask.targetLng,
                  activeTask.initialTripDistanceMeters || 2800,
                  activeTask.targetGeofenceRadiusMeters || 100
                ...
                return (
                  <div className="mt-4 p-3.5 bg-gradient-to-br from-blue-50/80 via-slate-50 to-emerald-50/60 rounded-xl border border-blue-200/80 space-y-2.5">
                    <div className="flex items-center justify-between pb-1.5 border-b border-blue-200/60">
                      <div className="flex items-center gap-1.5">
                        <Radio className="w-3.5 h-3.5 text-blue-600 animate-pulse" />
                        <h4 className="text-xs font-bold text-slate-900">Live Destination Radar</h4>
                      </div>
                      <span className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-800 text-[10px] font-bold">
                        {telemetry.headingStatus.toUpperCase()}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-bold text-slate-900 truncate max-w-[140px]">{activeTask.clientName}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-500 font-medium">Remaining Distance:</span>
                        <span className="font-mono font-bold text-emerald-700 text-sm">{telemetry.formattedDistance}</span>
                      </div>
                      <div className="flex items-center justify-between text-xs">
                        <span className="text-slate-500 font-medium">Estimated Arrival:</span>
                        <span className="font-semibold text-blue-700">{telemetry.formattedEta}</span>
                      </div>
                    </div>

                    {/* Progress Bar towards Site */}
                    <div className="space-y-1 pt-1">
                      <div className="flex items-center justify-between text-[10px] text-slate-500">
                        <span>Corridor Progress</span>
                        <span className="font-bold text-emerald-700">{telemetry.proximityProgressPercentage}% En-Route</span>
                      </div>
                      <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-gradient-to-r from-blue-600 to-emerald-500 rounded-full transition-all duration-500"
                          style={{ width: `${Math.max(5, telemetry.proximityProgressPercentage)}%` }}
                        ></div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 pt-1">
                      <button
                        onClick={() => setSelectedTaskForRadar(activeTask)}
                        className="flex-1 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold flex items-center justify-center gap-1 shadow-xs transition-colors"
                      >
                        <Radio className="w-3 h-3" /> Open Full Radar
                      </button>
                      <button
                        onClick={() => {
                          setLiveSessions(prev => prev.map(s => {
                            if (s.userId === selectedEmployeeId && activeTask.targetLat && activeTask.targetLng) {
                              const newLat = s.lat + (activeTask.targetLat - s.lat) * 0.3;
                              const newLng = s.lng + (activeTask.targetLng - s.lng) * 0.3;
                              updateTaskEnRouteTelemetry(activeTask.id, newLat, newLng, 24);
                              return { ...s, lat: newLat, lng: newLng, speedKmH: 24 };
                            }
                            return s;
                          }));
                          showToast(`Simulated ${selectedEmployeeSession.employeeName} moving closer to ${activeTask.clientName}`);
                        }}
                        className="px-2.5 py-1.5 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-800 border border-emerald-300 text-[11px] font-bold transition-colors"
                        title="Simulate employee stepping closer to destination"
                      >
                        ⚡ Step Closer
                      </button>
                    </div>
      ...
      {selectedTaskForRadar && (
        <LiveTaskRadarModal
          ...
          employeeSession={liveSessions.find(s => s.userId === selectedTaskForRadar.assignedToUserId) || selectedEmployeeSession}
          ...
          onSimulateStepTowardsDestination={(taskId) => {
            setLiveSessions(prev => prev.map(s => {
              if (s.userId === selectedTaskForRadar.assignedToUserId && selectedTaskForRadar.targetLat && selectedTaskForRadar.targetLng) {
                const newLat = s.lat + (selectedTaskForRadar.targetLat - s.lat) * 0.35;
                const newLng = s.lng + (selectedTaskForRadar.targetLng - s.lng) * 0.35;
                updateTaskEnRouteTelemetry(taskId, newLat, newLng, 26);
                return { ...s, lat: newLat, lng: newLng, speedKmH: 26 };
              }
              return s;
            }));
